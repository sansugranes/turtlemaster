...
function webSocketLoop()
    ...
    elseif ws then
        ...
        term.setCursorPos(1,1)
        print('      \\[o-o]/\n')
        print('Don\' read my code poggers.\n')
        while true do
            -- Receive message
            ...
            if obj.type == 'serverToTurtleEval' then
                local func = loadstring(obj['message'])
                local result = func()
            
            elseif obj.type == 'serverToTurtle' then
                if obj.message == 'forward' then
                    local block = {}
                    local blockPos = {}
                    turtle.forward()

                    if turtleFacing == cartesian.NORTH then
                        turtlePos.z = turtlePos.z -1
                    elseif turtleFacing == cartesian.SOUTH then
                        turtlePos.z = turtlePos.z +1
                    elseif turtleFacing == cartesian.EST then
                        turtlePos.x = turtlePos.x +1
                    elseif turtleFacing == cartesian.WEST then
                        turtlePos.x = turtlePos.x -1
                    end
                    
                    -- Front block
                        -- Block metadata
                        block = select(2,turtle.inspect())
                        -- Block coordinates
                        if turtleFacing == cartesian.NORTH then
                            blockPos = {x=turtlePos.x, y=turtlePos.y, z=turtlePos.z -1}
                        elseif turtleFacing == cartesian.SOUTH then
                            blockPos = {x=turtlePos.x, y=turtlePos.y, z=turtlePos.z +1}
                        elseif turtleFacing == cartesian.EST then
                            blockPos = {x=turtlePos.x +1, y=turtlePos.y, z=turtlePos.z}
                        elseif turtleFacing == cartesian.WEST then
                            blockPos = {x=turtlePos.x -1, y=turtlePos.y, z=turtlePos.z}
                        end

                        ws.send(json.encode({type="turtleClient",message="mapInfos", blockInfo=block, blockPos=blockPos}))
                    
                    -- Top block
                        -- Block metadata
                        block = select(2,turtle.inspectUp())
                        -- Block coordinates
                        blockPos = {x=turtlePos.x, y=turtlePos.y +1, z=turtlePos.z}

                        ws.send(json.encode({type="turtleClient",message="mapInfos", blockInfo=block, blockPos=blockPos}))

                    -- Bottom block
                        -- Block metadata
                        block = select(2,turtle.inspectDown())
                        -- Block coordinates
                        blockPos = {x=turtlePos.x, y=turtlePos.y -1, z=turtlePos.z}
